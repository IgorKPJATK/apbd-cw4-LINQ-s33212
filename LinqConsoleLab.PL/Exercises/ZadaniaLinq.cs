...
public sealed class ZadaniaLinq
{
    ...
    {
        var wszyscy = DaneUczelni.Prowadzacy
            .Count;
        
        var kadetry = DaneUczelni.Prowadzacy
            .Count(s => s.Katedra != null);

        yield return (wszyscy == kadetry) ? "Tak" : "Nie";
    ...
    public IEnumerable<string> Zadanie16_NajwyzszaOcenaKazdegoStudenta()
    {
        var method = DaneUczelni.Studenci
            .Join(DaneUczelni.Zapisy,
                ...
                (student, zapis) => new
                {
                    student.Imie, student.Nazwisko, zapis.OcenaKoncowa
                })
            .Where(s => s.OcenaKoncowa != null)
            .GroupBy(s => new { s.Imie, s.Nazwisko })
            .Select(s => $"{s.Key.Imie} {s.Key.Nazwisko}, {s.Max(s => s.OcenaKoncowa)}");
    ...
    public IEnumerable<string> Wyzwanie02_PrzedmiotyStartujaceWKwietniuBezOcenKoncowych()
    {
        var method  = DaneUczelni.Przedmioty
            .Join(DaneUczelni.Zapisy,
                ...
                (przedmiot,zapis) => new
                {
                    przedmiot.Nazwa, przedmiot.DataStartu, zapis.OcenaKoncowa
                })
            .Where(p => p.DataStartu.Month.Equals(4) && p.DataStartu.Year.Equals(2016))
            .GroupBy(p => p.Nazwa)
            .Where(s => s.All(z => z.OcenaKoncowa == null) )
            .Select(s => $"{s.Key}");
